...
void SplitContentAndStore(char *source, int nbdata, vector<int> startIndices, int dimension, const char separator, float* dest);
double CalculeCPUV1(float *pfData, int iRow, int iCol, int threads, const char* outputFileName);

/* GLOBALS */
std::string g_fileName = "C:\\Users\\frédéric.rayar\\Desktop\\2k14\\_tests\\_datasets\\mnist_d50.txt";
bool g_islabel = true;
ALGO g_ALGO = IRNG;
int g_NB_THREAD = 4;
EXPORT g_EXPORT_METHOD = JSON;
std::string g_graphOutputFilename = "C:\\Users\\frédéric.rayar\\Desktop\\2k14\\_tests\\_irng_omp_mnist_d50.json";
std::string g_edgesOutputFilename = "C:\\Users\\frédéric.rayar\\Desktop\\2k14\\_tests\\_irng_omp_mnist_d50.edges";
std::string g_distancesOutputFilename = "C:\\Users\\frédéric.rayar\\Desktop\\2k14\\_tests\\_irng_omp_mnist_d50.distances";
std::string g_timelogFilename = "C:\\Users\\frédéric.rayar\\Desktop\\2k14\\_tests\\_irng_omp_mnist_d50.timelog";
...
{
	ProcessFile(g_fileName.c_str(), g_islabel, '\t');
}
...
void ProcessFile(const char* fileName, bool label, const char separator)
{
	...
	float *data = new float[dimension * lines];
...
double CalculeCPUV1(float *pfData, int iRow, int iCol, int threads, const char* outputFileName){
	...
	// Attributes
	list<FloatResultPackage> *resultsCPU = NULL;
	double time0;
	float **distance;
	float distIJ,distIK,distJK;
	...
	FloatResultPackage resultPackage;
	resultsCPU = new list<FloatResultPackage>[iRow];

	// Set similarity matrix structure
	distance = new float*[iRow];
	for(int idxI = 0; idxI < iRow; ++idxI)
		distance[idxI] = new float[iRow];
