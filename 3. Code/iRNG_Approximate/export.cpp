 ...
 *
 */
void ExportGraph(list<FloatResultPackage>* pList, const char *filePath, int row, EXPORT method)
{
	ofstream ofsm(filePath , ios_base::trunc);
	for(int idxI = 0; idxI < row; ++idxI)
	{
		for(list<FloatResultPackage>::iterator it = pList[idxI].begin(); it != pList[idxI].end(); ++it)
			ofsm << (*it).Idx << '\t' << (*it).Distance << '\t';
		ofsm << endl;
	}
	ofsm.close();
}
 ...
 *
 */
void ExportGraph(std::map<__int64, float>* pList, const char *filePath, int row, EXPORT method){
	switch (method)
	{
		case ADJACENCIES:
 ...
 *
 */
void WriteAdjacencies(std::map<__int64, float>* pList, const char *filePath, int row){
	ofstream ofsm(filePath , ios_base::trunc);
	for(__int64 idxI = 0; idxI < row; ++idxI)
	{
		for(std::map<__int64, float>::iterator it = pList[idxI].begin(); it != pList[idxI].end(); it++)
			if (it->first > idxI)
 ...
 *
 */
void WriteJSONResults(std::map<__int64, float>* pList, const char *filePath, int row)
{
 ...
 *
 */
void WriteGEXFResults(std::map<__int64, float>* pList, const char *filePath, int row)
{
	ofstream ofsm(filePath , ios_base::trunc);
...
/**
 *
 * EXPORT RNG GEXF for TULIP
 *
 */
void WriteTLPResults(std::map<__int64, float>* pList, const char *filePath, int row)
{
	ofstream ofsm(filePath , ios_base::trunc);
